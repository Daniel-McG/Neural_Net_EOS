...
import re
import sys
path_to_results = '/Users/sanjaykumaran/Downloads/LJ-results'
Na = 6.02*10**23
#Kb = 1.380649*(10**-23)
Kb = 1 # Reduced Units?
...
def script(path_to_results):
    ...
    for (root,dirs,files) in os.walk(path_to_results, topdown=True):
        ...
        for filename in files:
            if (filename == NVT_results_filename):
                ...
                temp_and_density_foldername_string = str(temp_and_density_foldername)
                print(temp_and_density_foldername_string)

                # Use regex to find the temperature and density from the folder name
                temp_and_density = re.findall(r'tad(\d+\.\d+)_rhoad(\d+\.\d+)', temp_and_density_foldername_string)
                print(temp_and_density)
                # Unpack the temperature and density list into the temperature and density variables
                updated_list = [item for tup in temp_and_density for item in tup]
                temperature = updated_list[0]
                density = updated_list[1]
            ...
            if filename== NPT_results_filename:
                ...
                # Use regex to find the temperature and density from the folder name
                temp_and_density1 = re.findall(r'tad(\d+\.\d+)_rhoad(\d+\.\d+)', temp_and_density_foldername_string)
                updated_list1 = [item for tup in temp_and_density1 for item in tup]
                temperature1 = updated_list1[0]
                density1 = updated_list1[1]
                # Unpack the temperature and density list into the temperature and density variables

                # Convert temperature and density stings to floats
                initial_temperature = float(temperature1)
                initial_density = float(density1)
                ...
                if data_arr[:,0].sum() == 0:
                    continue
                if os.stat(path_to_results).st_size == 0:
                    continue
    ...
    np.savetxt("/Users/sanjaykumaran/Downloads/attempt1.txt",coallated_properties_with_removed_invalid_densities_and_temperatures)
