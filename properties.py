...
def isochoric_heat_capacity(N,PE,T):
    ...
    part1 = (Na/N)*((np.mean(squared_1 - squared_2))/(Kb*T**2))
    part2 = (Na/N)*(3/2)*(N*Kb)
    return part1 + part2
...
def isobaric_heat_capacity(E,N,P,V,T):
    ...
    return (Na/N)*(1/(Kb*(T**2)))*(np.mean(squared_1 - squared_2))
...
def thermal_expansion_coefficient(E,P,PE,T,V):
    ...
    return (1/(Kb*(T**2)*PE_average))*(np.mean((PE - PE_average)*(E_PV-E_PV_average)))

def isothermal_compressibility(PE,T):
    PE_average = np.mean(PE)
    squared_1 = np.mean(PE**2)
    squared_2 = PE_average**2
    return (1/(PE_average*Kb*T))*(np.mean(squared_1-squared_2))
