...
def script(path_to_results):
    ...
    for (root,dirs,files) in os.walk(path_to_results, topdown=True):
            ...
            collated_properties=np.append(collated_properties,[npt_nvt_derivative_results],axis=0)
            standard_deviations = np.concatenate((std_NPT_results,std_NVT_results))
            collated_standard_deviations = np.append(collated_standard_deviations,[standard_deviations],axis=0)
    ...
    density_difference_between_ensembles = collated_properties[:,density_nvt_column]-collated_properties[:,density_npt_column]
    absolute_density_difference_between_ensembles = np.abs(density_difference_between_ensembles)
    temperature_difference_between_ensembles = collated_properties[:,temperature_nvt_column]-collated_properties[:,temperature_npt_column]
    ...
    collated_properties_with_removed_invalid_densities_and_temperatures = np.delete(collated_properties, # From this array
                                                                    convergence_mask, # Delete any rows which violate the convergence criteria
                                                                    axis=0
                                                                    )
    strings_to_log = ["Number of runs before NPT NVT convergence check: {}".format(len(collated_properties)),
                      "Number of runs after NPT NVT convergence check: {}".format(len(collated_properties_with_removed_invalid_densities_and_temperatures))
                      ]
    
    with open('log.txt', 'w') as f:
        f.write("\n".join(strings_to_log))
        
    np.savetxt("local_collated_results_debug.txt",collated_properties_with_removed_invalid_densities_and_temperatures)
