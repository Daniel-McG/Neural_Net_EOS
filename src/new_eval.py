...
class BasicLightning(pl.LightningModule):
    ...
    def training_step(self, train_batch, batch_index):
        ...
        adiabatic_index_predicted = adiabatic_index_predicted[
            non_nan_adiabatic_index_index
        ]
        ...
        adiabatic_index_loss = (
            1
            / 20
            * ((adiabatic_index_target - adiabatic_index_predicted) ** 2)
            / var_adiabatic_index
        )
        ...
        rho_betaT_loss = (
            ...
            * (
                (
                    rho[non_nan_betaT_index] * betaT_target
                    - rho[non_nan_betaT_index] * betaT_predicted
                )
                ** 2
            )
            / var_rho_betaT
        ...
        self.log(
            "train_alphaP_loss", torch.mean((alphaP_target - alphaP_predicted) ** 2)
        )
        ...
        self.log(
            "train_adiabatic_index_loss",
            torch.mean((adiabatic_index_target - adiabatic_index_predicted) ** 2),
        )
    ...
    def compute_gradient(self, inputs, input_at_zero_densities):
        ...
        gradient = torch.autograd.grad(
            self.forward(inputs) - self.forward(input_at_zero_densities),
            inputs,
    ...
    def compute_hessian(self, input):
        ...
        hessians = torch.vmap(
            torch.func.hessian(self.compute_residual_helmholtz_for_hessian), (0)
        )(input)
        return hessians

    def calculate_variance(self, Tensor_to_calculate_variance):
        Tensor_to_calculate_variance = Tensor_to_calculate_variance[
            ~torch.isnan(Tensor_to_calculate_variance)
        ]
    ...
    def calculate_d2P_drho2(self, input):
        ...
        d2A_drho2 = self.calculate_d2A_drho2(input)
        d2A_drho2 = torch.reshape(d2A_drho2, (-1,))
        d3A_drho3 = torch.autograd.grad(
            d2A_drho2, input, grad_outputs=torch.ones_like(d2A_drho2), create_graph=True
        )[0][:, 0]
...
predicted_cv = (
    model.calculate_cv(val_inputs).detach().numpy()[~np.isnan(val_arr[:, cv_column])]
)
predicted_z = (
    model.calculate_Z(val_inputs).detach().numpy()[~np.isnan(val_arr[:, Z_column])]
)
predicted_U = (
    model.calculate_U(val_inputs)
    .detach()
    .numpy()[~np.isnan(val_arr[:, internal_energy_column])]
)
predicted_P = (
    model.calculate_P(val_inputs)
    .detach()
    .numpy()[~np.isnan(val_arr[:, pressure_column])]
)
predicted_alphaP = (
    model.calculate_alphaP(val_inputs)
    .detach()
    .numpy()[~np.isnan(val_arr[:, alphaP_column])]
)
predicted_betaT = (
    model.calculate_betaT(val_inputs)
    .detach()
    .numpy()[~np.isnan(val_arr[:, betaT_column])]
)
predicted_mujt = (
    model.calculate_mu_jt(val_inputs)
    .detach()
    .numpy()[~np.isnan(val_arr[:, mu_jt_column])]
)
predicted_gammaV = (
    model.calculate_gammaV(val_inputs)
    .detach()
    .numpy()[~np.isnan(val_arr[:, gammaV_column])]
)
predicted_cp = (
    model.calculate_cp(val_inputs).detach().numpy()[~np.isnan(val_arr[:, cp_column])]
)
...
target_U = val_arr[:, internal_energy_column][
    ~np.isnan(val_arr[:, internal_energy_column])
]
...
target_adiabatic_index = (
    target_cp / val_arr[:, cv_column][~np.isnan(val_arr[:, cp_column])]
)
...
scatter = ax[0].scatter(
    val_arr[:, density_column][~np.isnan(val_arr[:, Z_column])],
    val_arr[:, temperature_column][~np.isnan(val_arr[:, Z_column])],
    c=error,
    cmap=newcmp,
...
scatter = ax[1].scatter(
    val_arr[:, density_column][~np.isnan(val_arr[:, internal_energy_column])],
    val_arr[:, temperature_column][~np.isnan(val_arr[:, internal_energy_column])],
    c=error,
    cmap=newcmp,
    norm=matplotlib.colors.LogNorm(vmin=1e-3),
    s=marker_size,
...
sns.scatterplot(
    x=predicted_cv.flatten(), y=target_cv.flatten(), ax=axs[0, 3], alpha=0.2
)
...
sns.scatterplot(
    x=predicted_betaT.flatten(), y=target_betaT.flatten(), ax=axs[1, 0], alpha=0.2
)
sns.lineplot(
    x=[0, 400],
    y=[0, 400],
...
sns.scatterplot(
    x=predicted_cp.flatten(), y=target_cp.flatten(), ax=axs[0, 4], alpha=0.2
)
...
sns.scatterplot(
    x=predicted_alphaP.flatten(), y=target_alphaP.flatten(), ax=axs[1, 2], alpha=0.2
)
...
sns.scatterplot(
    x=predicted_gammaV.flatten(), y=target_gammaV.flatten(), ax=axs[1, 1], alpha=0.2
)
...
sns.scatterplot(
    x=predicted_mujt.flatten(), y=target_mujt.flatten(), ax=axs[1, 3], alpha=0.2
)
...
sns.scatterplot(
    x=predicted_adiabatic_index.flatten(),
    y=target_adiabatic_index.flatten(),
    ax=axs[1, 4],
    alpha=0.2,
)
...
for temperature in crit_isotherm_array:
    ...
    index_of_points_close_to_temp = find_closest(
        train_arr[:, temperature_column], temperature, 50
    )
    ...
    mask = (
        train_arr[index_of_points_close_to_temp, temperature_column] < upper_limit
    ) & (lower_limit < train_arr[index_of_points_close_to_temp, temperature_column])
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=predicted_cv,
        ax=isotherm_plots[0, 0],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=cv_isotherm_MD.flatten(),
        ax=isotherm_plots[0, 0],
        color=colour_list[i],
    )
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=predicted_cp,
        ax=isotherm_plots[0, 1],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=cp_isotherm_MD.flatten(),
        ax=isotherm_plots[0, 1],
        color=colour_list[i],
    )
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=predicted_mujt,
        ax=isotherm_plots[0, 2],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=mu_jt_isotherm_MD.flatten(),
        ax=isotherm_plots[0, 2],
        color=colour_list[i],
    )
    isotherm_plots[0, 2].set_xlabel(r"$\mathit{\rho^*}$", style="italic", fontsize=18)
    isotherm_plots[0, 2].set_ylabel(
        r"$\mathit{\mu_{JT}^*}$", style="italic", fontsize=18
    )
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=predicted_gammaV,
        ax=isotherm_plots[1, 0],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=gammaV_isotherm_MD.flatten(),
        ax=isotherm_plots[1, 0],
        color=colour_list[i],
    )
    isotherm_plots[1, 0].set_xlabel(r"$\mathit{\rho^*}$", style="italic", fontsize=18)
    isotherm_plots[1, 0].set_ylabel(
        r"$\mathit{\gamma_v^*}$", style="italic", fontsize=18
    )
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=predicted_alphaP,
        ax=isotherm_plots[1, 1],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=alphaP_isotherm_MD.flatten(),
        ax=isotherm_plots[1, 1],
        color=colour_list[i],
    )
    isotherm_plots[1, 1].set_xlabel(r"$\mathit{\rho^*}$", style="italic", fontsize=18)
    isotherm_plots[1, 1].set_ylabel(
        r"$\mathit{\alpha_p^*}$", style="italic", fontsize=18
    )
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=density.numpy() * predicted_betaT,
        ax=isotherm_plots[1, 2],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=density_MD.flatten() * betaT_isotherm_MD.flatten(),
        ax=isotherm_plots[1, 2],
        color=colour_list[i],
    )
    isotherm_plots[1, 2].set_xlabel(r"$\mathit{\rho^*}$", style="italic", fontsize=18)
    isotherm_plots[1, 2].set_ylabel(
        r"$\mathit{\rho^* \beta_T^*}$", style="italic", fontsize=18
    )
...
for temperature in crit_isotherm_array:
    ...
    index_of_points_close_to_temp = find_closest(
        train_arr[:, temperature_column], temperature, 100
    )
    ...
    sns.lineplot(
        x=density.numpy().flatten(),
        y=predicted_P,
        ax=P_isotherm_plots[0],
        color=colour_list[i],
    )
    sns.scatterplot(
        x=density_MD.flatten(),
        y=P_isotherm_MD.flatten(),
        ax=P_isotherm_plots[0],
        color=colour_list[i],
    )
...
print(
    "Equilibrium liquid density at T={Tsat}: ".format(Tsat=T[i]), np.round(rhol[i], 5)
)
print(
    "Equilibrium vapour density at T={Tsat}: ".format(Tsat=T[i]), np.round(rhov[i], 5)
)
...
rhov = np.zeros(n)  # array to store vapour density
T = np.array(
    [0.415, 0.425, 0.435, 0.445, 0.450, 0.455, 0.460, 0.465, 0.468]
)  # array with the saturation temperatures
...
ROS_EOS_VLE_df = pd.read_csv(
    r"C:\Users\Daniel.000\Documents\New_research_project\Neural_Net_EOS\Lit_Data\RO_EOS_VLE_DATA.txt",
    delimiter=" ",
)
CMO_EOS_VLE_df = pd.read_csv(
    r"C:\Users\Daniel.000\Documents\New_research_project\Neural_Net_EOS\Lit_Data\CMO_EOS_VLE_DATA.txt",
    delimiter=" ",
)
# Literature VLE simulation Dataw

vle_temp_2019_data = np.array([0.4, 0.41, 0.42, 0.43, 0.44])
inverse_gas_density_2019_data = np.array(
    [35.40806914, 28.16798258, 21.91249159, 16.245217, 11.06553708]
)
inverse_liq_density_2019_data = np.array(
    [1.331229281, 1.361101621, 1.39302346, 1.436343071, 1.480891538]
)
...
density_gas2 = np.array(
    [0.0163, 0.0173, 0.0235, 0.0274, 0.0285, 0.0339, 0.0406, 0.0473, 0.0420]
)
...
vle_liq_df = pd.DataFrame({"Densities": liq_vle_density, "Temperatures": liq_vle_temp})
...
vle_gas_df = pd.DataFrame(
    {"Densities": gas_vle_density, "Temperatures": gas_vle_temperature}
)
...
plots.scatter(
    x=critical_point_density,
    y=critical_point_temperature,
    facecolors="r",
    edgecolors="r",
    marker="*",
    zorder=1,
...
plots.plot(
    ROS_EOS_VLE_df["rhoV"].values, ROS_EOS_VLE_df["T"].values, label="RO EOS", color="b"
)
plots.plot(
    ROS_EOS_VLE_df["rhoL"].values, ROS_EOS_VLE_df["T"].values, label="RO EOS", color="b"
)
plots.plot(
    CMO_EOS_VLE_df["rhoV"].values,
    CMO_EOS_VLE_df["T"].values,
    label="CMO EOS",
    color="g",
)
plots.plot(
    CMO_EOS_VLE_df["rhoL"].values,
    CMO_EOS_VLE_df["T"].values,
    label="CMO EOS",
    color="g",
)
